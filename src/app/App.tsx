...
export const App = () => {
  ...
  const driveUntilObstacleAndReturn = async () => {
    if (!isConnected) {
      return;
    }
    ...
    rateLimiter.stop();
    try {
      await bleClient.driveUntilObstacleAndReturn(AUTONOMY_OBSTACLE_STOP_DISTANCE_MM);
    } catch (error) {
      setDisconnected(error instanceof BleClientError ? error.category : 'write-failed');
    }
  };
